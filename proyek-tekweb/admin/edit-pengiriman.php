...
if (isset($_POST['edit'])) {
    $jenis_pengiriman = isset($_POST['jenis_pengiriman']) ? trim($_POST['jenis_pengiriman']) : '';
    $deskripsi = trim($_POST['deskripsi']);
    $harga = trim($_POST['harga']);

    if (!empty($jenis_pengiriman)) {
        $update_data = $admin->update_pengiriman([
            'id' => $id,
        ...
        if ($update_data) {
            $msg = 'Berhasil mengedit data pengiriman.';
            header("Location: ../admin/table-pengiriman.php");
            exit();
        } else {
            $msg = 'Gagal mengedit data pengiriman.';
        }
    } else {
        $msg = 'Jenis Pengiriman cannot be empty.';
    }
}
...
<body>
    <div class="main">
        <nav class="navbar-bg">
            <div class="navbar">
                <ul class="navbar-list">
                    ...
                    <li><a href="../input_barang/inputbrg.php" id="inputbrg">Add Product</a></li>
        ...
        <div class="header">
            <label>EDIT PENGIRIMAN</label>
        </div>

        <div class="container">
            <form action="#" method="post">
                <?= (isset($_GET['msg']) && $_GET['msg'] == 'Berhasil mengedit data customer') ? '<div class="alert alert-success">Data berhasil ditambahkan</div>' : '' ?>

                <label for="name">Jenis Pengiriman:</label>
                <input type="text" name="jenis_pengiriman" required>

                <label for="name">Deskripsi:</label>
                <input type="text" name="deskripsi" required>

                <label for="name">Harga:</label>
                <input type="text" id="name" name="harga" required>

                <div class="d-grid gap-2">
                    <a href="table-pengiriman.php" class="btn btn-danger">&laquo; Cancel</a>
