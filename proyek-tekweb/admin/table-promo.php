...
$searchTerm = isset($_GET['cari_promo']) ? $_GET['cari_promo'] : '';
$customers = $admin->search_promo($searchTerm)->fetchAll(PDO::FETCH_ASSOC);

if (isset($_GET['id']) && count($_GET)) {
    $delete_data = $admin->delete_promo($_GET['id']);

    if ($delete_data) {
        header('location: table-promo.php');
    } else
        $msg = 'eror';
}
...
<body>
    <div class="main">
        <nav class="navbar-bg">
            <div class="navbar">
                <ul class="navbar-list">
                    ...
                    <li><a href="#" id="promotable">Promo Table</a></li>
        ...
        <div class="header">
            <label>PROMO TABLE</label>
        </div>

        <div class="container">
            <div class="table-title">
                <a href="../admin/addpromo.php" class="btn btn-primary btn-sm">Add Promo</a>
                <form class="search-form" method="get">
                    <div class="input-group">
                        <input type="text" class="form-control" placeholder="Search by promo name" name="cari_promo" value="<?= htmlspecialchars($searchTerm) ?>">
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    <a href="edit-promo.php?id=<?= $data['id'] ?>" class="btn btn-primary btn-sm">Edit</a>
                                    <form method="post" action="" id="deleteForm<?= $data['id'] ?>" style="display: inline;">
                                        <input type="hidden" name="id" value="<?= $data['id'] ?>">
                                        <a href="table-promo.php?id=<?= $data['id'] ?>" class="btn btn-danger btn-sm" onclick="confirmDelete(<?= $data['id'] ?>)"> delete</a>
                                    </form>
                                </td>
                            </tr>
                    <?php
